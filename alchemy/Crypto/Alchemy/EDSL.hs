...
pt2 :: forall t m zp d ptexpr a . (AddPT ptexpr, MulPT ptexpr, a ~ Cyc t m zp,
 AddPubCtxPT ptexpr d a, AdditiveCtxPT ptexpr (Add1 d) a,
 RingCtxPT ptexpr d a, Ring a, LambdaD ptexpr)
  => ptexpr ('L (Add1 d) ('L (Add1 d) d)) (a -> a -> a)
pt2 = lamD $ lamD . pt1

pt3 :: forall t m zp d ptexpr a .
...
-}
tunn1 :: forall t r u s zp d expr eru eus .
  (TunnelPTCtx' expr d t eru r u zp,
   TunnelPTCtx' expr d t eus u s zp,
   LambdaD expr)
  => Proxy u -> expr ('L d d) (Cyc t r zp -> Cyc t s zp)
...
main = do
  ...
  putStrLn $ unSPT $ pt2 @CT @F4 @Int64 @('T 'Z)
  -- apply the PT function to arguments, then print it out
  putStrLn $ unSPT $ pt3 @CT @F4 @Int64 @('T 'Z) 7 11
  -- apply the PT function to arguments and evaluate the function
  putStrLn $ show $ unID $ pt3 @CT @F4 @Int64 @('T 'Z) 7 11
...
tunnelPT' :: forall s expr t r zp e d . (TunnelPTCtx' expr d t e r s zp)
